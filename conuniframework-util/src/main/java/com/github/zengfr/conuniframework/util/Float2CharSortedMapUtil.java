package com.github.zengfr.conuniframework.util;
import java.util.*;
import java.util.Map.Entry;
import com.google.common.collect.Table;
import com.google.common.collect.Table.Cell;
/**
* @author zengfr
* @author https://github.com/zengfr/conuniframework-util
* @author https://gitee.com/zengfr/conuniframework-util
* @author  http://blog.csdn.net/zfrong/
* @author zengfr3000@qq.com
*/
public final class Float2CharSortedMapUtil{ 
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#fastIterable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#fastIterable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*<code>public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterable<it.unimi.dsi.fastutil.floats.Float2CharMap$Entry> it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.fastIterable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*fastIterable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterable<it.unimi.dsi.fastutil.floats.Float2CharMap.Entry> fastIterable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.fastIterable(p0);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#fastIterator(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#fastIterator(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*<code>public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterator<it.unimi.dsi.fastutil.floats.Float2CharMap$Entry> it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.fastIterator(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*fastIterator(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterator<it.unimi.dsi.fastutil.floats.Float2CharMap.Entry> fastIterator(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.fastIterator(p0);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(float,char)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(float,char)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(float,char)
*singleton(float p0,char p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap singleton(float p0,char p1){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(java.lang.Float,java.lang.Character,it.unimi.dsi.fastutil.floats.FloatComparator)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(java.lang.Float,java.lang.Character,it.unimi.dsi.fastutil.floats.FloatComparator)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(java.lang.Float,java.lang.Character,it.unimi.dsi.fastutil.floats.FloatComparator)
*singleton(java.lang.Float p0,java.lang.Character p1,it.unimi.dsi.fastutil.floats.FloatComparator p2)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap singleton(java.lang.Float p0,java.lang.Character p1,it.unimi.dsi.fastutil.floats.FloatComparator p2){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(p0,p1,p2);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(java.lang.Float,java.lang.Character)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(java.lang.Float,java.lang.Character)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(java.lang.Float,java.lang.Character)
*singleton(java.lang.Float p0,java.lang.Character p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap singleton(java.lang.Float p0,java.lang.Character p1){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(float,char,it.unimi.dsi.fastutil.floats.FloatComparator)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#singleton(float,char,it.unimi.dsi.fastutil.floats.FloatComparator)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(float,char,it.unimi.dsi.fastutil.floats.FloatComparator)
*singleton(float p0,char p1,it.unimi.dsi.fastutil.floats.FloatComparator p2)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap singleton(float p0,char p1,it.unimi.dsi.fastutil.floats.FloatComparator p2){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.singleton(p0,p1,p2);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap,java.lang.Object)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap,java.lang.Object)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap,java.lang.Object)
*synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0,java.lang.Object p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0,java.lang.Object p1){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.synchronize(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap synchronize(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.synchronize(p0);
}
/**
*{@link it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#unmodifiable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)}
*@see it.unimi.dsi.fastutil.floats.Float2CharSortedMaps#unmodifiable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*<code>public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.unmodifiable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap)
*unmodifiable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.floats.Float2CharSortedMap unmodifiable(it.unimi.dsi.fastutil.floats.Float2CharSortedMap p0){
	return it.unimi.dsi.fastutil.floats.Float2CharSortedMaps.unmodifiable(p0);
}
}
