package com.github.zengfr.conuniframework.util;
import java.util.*;
import java.util.Map.Entry;
import com.google.common.collect.Table;
import com.google.common.collect.Table.Cell;
/**
* @author zengfr
* @author https://github.com/zengfr/conuniframework-util
* @author https://gitee.com/zengfr/conuniframework-util
* @author  http://blog.csdn.net/zfrong/
* @author zengfr3000@qq.com
*/
public final class Long2ByteSortedMapUtil{ 
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#fastIterable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#fastIterable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*<code>public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterable<it.unimi.dsi.fastutil.longs.Long2ByteMap$Entry> it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.fastIterable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*fastIterable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterable<it.unimi.dsi.fastutil.longs.Long2ByteMap.Entry> fastIterable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.fastIterable(p0);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#fastIterator(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#fastIterator(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*<code>public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterator<it.unimi.dsi.fastutil.longs.Long2ByteMap$Entry> it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.fastIterator(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*fastIterator(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.objects.ObjectBidirectionalIterator<it.unimi.dsi.fastutil.longs.Long2ByteMap.Entry> fastIterator(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.fastIterator(p0);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(long,byte)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(long,byte)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(long,byte)
*singleton(long p0,byte p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap singleton(long p0,byte p1){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(java.lang.Long,java.lang.Byte,it.unimi.dsi.fastutil.longs.LongComparator)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(java.lang.Long,java.lang.Byte,it.unimi.dsi.fastutil.longs.LongComparator)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(java.lang.Long,java.lang.Byte,it.unimi.dsi.fastutil.longs.LongComparator)
*singleton(java.lang.Long p0,java.lang.Byte p1,it.unimi.dsi.fastutil.longs.LongComparator p2)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap singleton(java.lang.Long p0,java.lang.Byte p1,it.unimi.dsi.fastutil.longs.LongComparator p2){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(p0,p1,p2);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(java.lang.Long,java.lang.Byte)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(java.lang.Long,java.lang.Byte)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(java.lang.Long,java.lang.Byte)
*singleton(java.lang.Long p0,java.lang.Byte p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap singleton(java.lang.Long p0,java.lang.Byte p1){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(long,byte,it.unimi.dsi.fastutil.longs.LongComparator)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#singleton(long,byte,it.unimi.dsi.fastutil.longs.LongComparator)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(long,byte,it.unimi.dsi.fastutil.longs.LongComparator)
*singleton(long p0,byte p1,it.unimi.dsi.fastutil.longs.LongComparator p2)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap singleton(long p0,byte p1,it.unimi.dsi.fastutil.longs.LongComparator p2){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.singleton(p0,p1,p2);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap,java.lang.Object)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap,java.lang.Object)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap,java.lang.Object)
*synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0,java.lang.Object p1)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0,java.lang.Object p1){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.synchronize(p0,p1);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap synchronize(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.synchronize(p0);
}
/**
*{@link it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#unmodifiable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)}
*@see it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps#unmodifiable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*<code>public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.unmodifiable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap)
*unmodifiable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0)</code>
*@author zengfr
*@author https://github.com/zengfr/conuniframework-util
*@author https://gitee.com/zengfr/conuniframework-util
*@author http://blog.csdn.net/zfrong/
*/ 
public static it.unimi.dsi.fastutil.longs.Long2ByteSortedMap unmodifiable(it.unimi.dsi.fastutil.longs.Long2ByteSortedMap p0){
	return it.unimi.dsi.fastutil.longs.Long2ByteSortedMaps.unmodifiable(p0);
}
}
