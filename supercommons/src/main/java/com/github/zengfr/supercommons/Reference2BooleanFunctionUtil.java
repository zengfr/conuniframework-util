package com.github.zengfr.supercommons;
import java.util.*;
import java.util.Map.Entry;
/**
* @author zengfr
* https://github.com/zengfr/Super-Commons
* http://blog.csdn.net/zfrong/
* zengfr3000@qq.com
*/
public final class Reference2BooleanFunctionUtil{ 
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.primitive(java.util.function.Function<? super K, ? extends java.lang.Boolean>)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> primitive(java.util.function.Function<? super K, ? extends java.lang.Boolean> p0){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.primitive(p0);
}
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.singleton(K,java.lang.Boolean)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> singleton(K p0,java.lang.Boolean p1){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.singleton(p0,p1);
}
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.singleton(K,boolean)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> singleton(K p0,boolean p1){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.singleton(p0,p1);
}
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.synchronize(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K>,java.lang.Object)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> synchronize(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> p0,java.lang.Object p1){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.synchronize(p0,p1);
}
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.synchronize(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K>)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> synchronize(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> p0){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.synchronize(p0);
}
/**
*public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.unmodifiable(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K>)
*/ 
public static <K> it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> unmodifiable(it.unimi.dsi.fastutil.objects.Reference2BooleanFunction<K> p0){
	return it.unimi.dsi.fastutil.objects.Reference2BooleanFunctions.unmodifiable(p0);
}
}
